...
void driver( const GV& gv, 						// GridView
	...
	//	MATERIAL PROPERTIES, NUMERICAL AND TEST PARAMETERS, CONSTITUTIVE RELATIONSHIPS
		...
		std::string pathName = "/sfs/fs1/work-geomar5/smomw325/dune_outputs/dune_2_8/Pockmark2P2C/case2_Test0_2D/";
		...
		RF dt 	  = ptree.get("time.dt_initial",(double)0.001);
		dt 		 *= 1./property.characteristicValue.t_c;
		RF t_END  = ptree.get("time.time_end",(double)86400.);
		t_END 	 *= 1./property.characteristicValue.t_c;
		// output time interval
		RF t_OP   = ptree.get("output.time_interval",(double)1.);
		t_OP 	 *= 1./property.characteristicValue.t_c;
...
//		PDESOLVER pdesolver( igo, ls, "noLineSearch" );
...
//		pdesolver.setParameters(ptree.sub("newton"));
	...
	//  POST-PROCESS
		...
		VTKSEQUENCEWRITER vtkSequenceWriter( gv,fileName,pathName,"",Dune::VTK::conforming);
